...
import {  useStartChangePatient, useUpdateMachineMutate } from "api/machines/machines.query";
...
export const MachineTableRow:React.FC<IMachineRowProps>=(props)=>{
    ...
    return(
         <TableRow>
                  <TableCell className="text-center w-[150px]">{props.machine.id}</TableCell>
                      ...
                      <Button
                        ...
                        onClick={async() => {
                         changeMachineId(props.machine.id)
                          try{
                          const res=await startChangePatient(props.machine.id)
                          changeLockId(res)
                          }
                          catch(err){
                            alert(err)
                  ...
                  <TableCell className="text-center w-[150px]">
                    <Button onClick={() => {
                        ...
                        if(updatedName!==props.machine.name||updatedLocation!==props.machine.location){
